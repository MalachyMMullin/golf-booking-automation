"""Golf booking automation script for PythonAnywhere (Sunday variant).
...
def compute_target_date() -> Tuple[datetime, datetime, datetime, str, str]:
    ...
    days_until_fri = (4 - local_now.weekday() + 7) % 7
    this_fri = local_now + timedelta(days=days_until_fri)
    target = this_fri + timedelta(days=9)
    dayname = target.strftime("%a")
    try:
        combo = target.strftime("%-d %b")
    except Exception:
        combo = target.strftime("%d %b").lstrip("0")
    return local_now, this_fri, target, dayname, combo


local_now, upcoming_friday, target_date, target_day_name, target_date_combo = compute_target_date()
log("--- RUNNING IN LIVE AUTOMATIC MODE [Sunday target] ---")
try:
...
log(f"The next booking Friday is: {upcoming_friday.strftime('%Y-%m-%d')}")
log(
    "Therefore, the script is targeting Sunday: "
...
def main() -> None:
    log("--- Golf Booking Bot Initialized [v51 modal-validated + verify rows | Sunday] ---")
    driver = make_driver()
